...
import {
  ...
  TrendingUp, Award, Zap, FileText, Hash, HelpCircle,
    ...
    <div style={{ position: "relative", display: "inline-flex", alignItems: "center" }}
      ...
      {show && (
        <div style={{
          ...
          color: "white", fontSize: "11px", borderRadius: "6px", zIndex: 1000,
    ...
    <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: "4px",
          backgroundColor: isHovered && !isLoading ? "#f1f5f9" : "transparent"
        }}
      >
...
export default function GenerateReport() {
  ...
  // NEW: capture filtered/sorted table state for PDF export
  const [publicationsView, setPublicationsView] = useState(null);

  // NEW: ref to the hidden "PDF layout" DOM
  ...
  // ✅ NEW: PDF export handler (uses filtered/sorted papers if filters applied)
  const handleDownloadPdf = async () => {
    if (!reportData || !pdfRef.current) return;

    const safeName = (reportData.profile?.name || "researcher")
      .replace(/[^\w\s-]/g, "")
      .trim()
      .replace(/\s+/g, "_");

    const filename = `${safeName}_publication_report.pdf`;
  ...
  // FILTERED papers (from table) for the PDF table only.
  const allPapers = reportData?.papers || [];
  const tablePapers = publicationsView?.sortedPapers ?? allPapers;
  ...
  return (
    <div style={{ width: "100%", backgroundColor: "#F8FAFC", minHeight: "100vh", paddingBottom: "60px", fontFamily: "Inter, -apple-system, sans-serif" }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", paddingTop: "40px" }}>
                ...
                {recents.length > 0 && (
                  <div style={{ marginBottom: "50px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "16px" }}>
                      {recents.map((profile) => (
                        <SavedProfileCard key={profile.id} profile={profile} onClick={() => handleRecentClick(profile.id)} />
                      ))}
                    </div>
                  </div>
                )}
          ...
          <div className="animate-fade-in">
            ...
            <div style={{ display: "flex", flexDirection: "column", marginBottom: "20px", borderBottom: "1px solid #E2E8F0", paddingBottom: "24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "2px" }}>
                ...
                <button
                  ...
                  title="Download PDF (includes graphs + productivity + filtered publications table)"
                >
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "32px" }}>
                  <div style={{ backgroundColor: "white", borderRadius: "12px", border: "1px solid #E2E8F0", padding: "24px", boxShadow: "0 1px 2px rgba(0,0,0,0.02)" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <div style={{ width: "1px", height: "40px", backgroundColor: "#d9dee5ff" }} />
            ...
            {activeTab === "publications" && (
              <div className="animate-fade-in">
                ...
                {/* ✅ Pass callback so GenerateReport knows current filtered/sorted view */}
                <PublicationsTable
                  papers={reportData.papers || []}
                  onViewChange={setPublicationsView}
                />
              </div>
            )}
          </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "center", gap: "6px" }}>
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        size={24}
                        fill={(hoverRating || rating) >= star ? "#f5bb0bff" : "transparent"}
                        color={(hoverRating || rating) >= star ? "#f5bb0bff" : "#CBD5E1"}
        ...
        {/* ✅ HIDDEN PDF LAYOUT (rendered offscreen but still captured) */}
        {reportData && (
          <div style={{ position: 'absolute', left: '-99999px', top: 0, width: '8.5in', backgroundColor: 'white' }}>
            <div ref={pdfRef} style={{ padding: '0.55in', fontFamily: 'Inter, -apple-system, sans-serif', color: '#0F172A' }}>

              {/* PRETTIER HEADER (same content, nicer spacing) */}
              <div style={{
                border: '1px solid #E2E8F0',
                borderRadius: 14,
                padding: 16,
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 12.5, color: '#64748B', marginTop: 5, lineHeight: 1.35 }}>
                      {reportData.profile.affiliations || "No affiliation listed"}
                    </div>
                    <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 8 }}>
                      Exported: {new Date().toLocaleString()}
                      {publicationsView?.filters && (
                        <>
                          {" • "}Filters:{" "}
                          {[
                            publicationsView.filters.filterVenue ? `Venue=${publicationsView.filters.filterVenue}` : null,
                            publicationsView.filters.filterType ? `Type=${publicationsView.filters.filterType}` : null,
                            publicationsView.filters.filterRank ? `Rank=${publicationsView.filters.filterRank}` : null,
                            publicationsView.filters.filterRecent ? `Last 5 Years` : null,
                  ...
                  {/* Small meta chips (same info not added; just reuse existing ID and academic age if you want)
                      CONTENT RULE: not adding new content fields — leaving this area blank on purpose. */}
                </div>
              </div>

              {/* Visualisations (ALWAYS unfiltered) - stacked one after another */}
              <div style={{ pageBreakInside: 'avoid' }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: "0.02em", textTransform: "uppercase", color: "#475569" }}>
                ...
                <div style={{
                  ...
                  <PublicationAnalytics papers={allPapers} layout="stack" />
                </div>
              </div>
              ...
              <div style={{ pageBreakInside: 'avoid' }}>
                <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: "0.02em", textTransform: "uppercase", color: "#475569", marginBottom: 10 }}>
                  Productivity
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ transform: "scale(0.92)", transformOrigin: "top left" }}>
                    <ProductivityChart papers={allPapers} height={220} />
                  </div>
                </div>
              </div>

              {/* Force table to start on a NEW page so it doesn't get cut */}
              <div style={{ pageBreakBefore: 'always', breakBefore: 'page', marginTop: 18 }}>
                ...
                <div style={{
                  ...
                  <PdfPublicationsTable papers={tablePapers} />
                </div>

                {/* subtle footer line (no new content) */}
                <div style={{ marginTop: 14, fontSize: 10.5, color: "#94A3B8", textAlign: "right" }}>
                  {/* intentionally blank content-wise */}
                </div>
              </div>
...
// --- PDF TABLE (simple HTML table for clean print) ---
function PdfPublicationsTable({ papers }) {
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10.2 }}>
      <thead>
        <tr style={{ backgroundColor: "#F8FAFC" }}>
          <th style={pdfTh}>Title</th>
          <th style={pdfTh}>Venue</th>
          <th style={pdfThRight}>Year</th>
          <th style={pdfThRight}>Rank</th>
          <th style={pdfThRight}>Author</th>
          <th style={pdfThRight}>Cites</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((p, idx) => (
          <tr key={idx} style={{ pageBreakInside: 'avoid' }}>
            <td style={{ ...pdfTd, fontWeight: 700, color: "#0F172A" }}>{p.title || "-"}</td>
            <td style={pdfTd}>{p.venue || "Unknown"}</td>
            <td style={pdfTdRight}>{p.year || "-"}</td>
            <td style={pdfTdRight}>{p.rank || "-"}</td>
            <td style={pdfTdRight}>{p.author_pos || "-"}</td>
            <td style={pdfTdRight}>{p.citations ?? 0}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
const pdfThRight = { ...pdfTh, textAlign: 'right' };
const pdfTd = {
  textAlign: 'left',
  padding: '9px 10px',
...
const pdfTdRight = { ...pdfTd, textAlign: 'right', whiteSpace: 'nowrap', fontWeight: 700 };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: "4px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "5px", width: "100%" }}>
        <span style={{ fontSize: "12px", fontWeight: "700", color: "#64748B", textTransform: labelTransform, letterSpacing: "0.02em" }}>
          {label}
        </span>
      ...
      {sub && (
        <span style={{ fontSize: "11px", color: "#94A3B8", fontWeight: "500", textAlign: "center", marginTop: "-3px" }}>
          {sub}
        </span>
      )}
    </div>
...
function SavedProfileCard({ profile, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ backgroundColor: "white", padding: "16px", borderRadius: "12px", border: "1px solid #E2E8F0", cursor: "pointer", transition: "all 0.2s ease", display: "flex", flexDirection: "column", gap: "10px", boxShadow: "0 1px 2px rgba(0,0,0,0.04)" }}
