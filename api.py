...
# 1. Define where the React build files are located
# On Render, they are likely in 'frontend/dist'
DIST_DIR = os.path.join(os.path.dirname(__file__), "frontend", "dist")

# Check if the build folder actually exists (prevents crashes locally if not built)
if os.path.exists(DIST_DIR):
    
    # A. Mount the 'assets' folder (CSS, JS, images)
    # This allows http://yoursite.com/assets/index.js to work
    app.mount("/assets", StaticFiles(directory=os.path.join(DIST_DIR, "assets")), name="assets")

    # B. Catch-All Route for SPA (Single Page Application)
    # If a user visits /dashboard or /login, we serve index.html and let React handle the routing.
    @app.get("/{full_path:path}")
    async def serve_react_app(full_path: str):
        
        # 1. Check if the requested file exists in dist (e.g., logo.png, vite.svg)
        file_path = os.path.join(DIST_DIR, full_path)
        if os.path.exists(file_path) and os.path.isfile(file_path):
            return FileResponse(file_path)
            
        # 2. Otherwise, return index.html (React handles the 404s or routes)
    ...
    @app.get("/")
    def home():
        return {"message": "API running, but React frontend not found. Did you run 'npm run build'?"}